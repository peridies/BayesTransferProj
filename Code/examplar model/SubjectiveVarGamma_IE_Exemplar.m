%compute exemplar model variance data
%house keeping
clear all;
close all;
...
load('Learning4Stats_Good_P.mat','exSubj','inSubj','nExS',...
    ...
    'nCond','LLSd','Wop','PrSD');
load('Exemplar4Stats_P.mat','DataTable') 
end 
%% data table
subj = T.Participant;
type = T.LLType;
...
erex_sPSdPn1 = sqrt(std(sPSdpn1).^2/nExS);
erex_sPSdPm = sqrt(std(sPSdpm).^2/nExS);

% [h1r,p1r] = ttest(sPSdrn1,sPSdrm)
mex_sPSdRn1 = mean(sPSdrn1,"omitnan")%subplot1 right
mex_sPSdRm = mean(sPSdrm,"omitnan")
erex_sPSdRn1 = sqrt(std(sPSdrn1).^2/nExS);%er3
erex_sPSdRm = sqrt(std(sPSdrm).^2/nExS);%er4

% [h2l,p2l] = ttest(sPSdpn2,sPSdpw)
min_sPSdPn2 = mean(sPSdpn2,"omitnan")%subplot2 left (inter)
min_sPSdPw = mean(sPSdpw,"omitnan")
erin_sPSdPn2 = sqrt(std(sPSdpn2).^2/nInS);
erin_sPSdPw = sqrt(std(sPSdpw).^2/nInS);
...
erin_sPSdRn2 = sqrt(std(sPSdrn2).^2/nInS);%er1
erin_sPSdRw = sqrt(std(sPSdrw).^2/nInS);%er2
...
subplot(1,2,1);
hold on;
b1 = bar(xt(1),min_sPSdRn2,0.3,'FaceColor',[.2 .2 .2])
er1 = errorbar(xt(1),min_sPSdRn2,erin_sPSdRn2,erin_sPSdRn2);
er1.Color = [0 0 0];                            
er1.LineStyle = 'none'; 
er1.LineWidth = 1; 

b2 = bar(xt(2),min_sPSdRw,0.3,'FaceColor',[.45 .45 .45])
er2 = errorbar(xt(2),min_sPSdRw,erin_sPSdRw,erin_sPSdRw);
er2.Color = [0 0 0];                            
er2.LineStyle = 'none'; 
er2.LineWidth = 1; 

b3 = bar(xt(3),mex_sPSdRn1,0.3,'FaceColor',[.2 .2 .2])
er3 = errorbar(xt(3),mex_sPSdRn1,erex_sPSdRn1,erex_sPSdRn1);
er3.Color = [0 0 0];                            
er3.LineStyle = 'none'; 
er3.LineWidth = 1; 

b4 = bar(xt(4),mex_sPSdRm,0.3,'FaceColor',[.75 .75 .75])
...
b6 = bar(xt(2),min_sPSdPw,0.3,'FaceColor',[.45 .45 .45])
...
%%
% save('Exemplar_variances1_P.mat','sLSd','sLVar',...
