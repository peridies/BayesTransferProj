...
%learning phase, remove first 25 trials
%%house keeping
clear all;
close all;
...
for i = 1:nSj
    for k = 1:nType %1: narrow, 2: medium, 3: wide
        ...
        useTmpP = useTmp(26:end); %the only change => remove first 25 trials
        if pickGood == 0
        useTr = useTmpP;
        else
        useTr = intersect(useTmpP,find(contains(T.Outlier,'no')));
        end
...
%% save data
% if pickGood == 1
% save('Learning4Stats_Good_P.mat');